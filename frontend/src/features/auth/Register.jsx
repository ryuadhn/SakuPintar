...
export default function Register() {
  ...
  const { user, register, login, loginWithGoogle, demo, authError } = useAuth();
  const formId = useId();
  ...
  return (
    <main className="ui-auth-shell ui-auth-compact ui-auth-register">
      <section className="ui-auth-card" aria-labelledby={`${formId}-title`}>
        <header className="ui-auth-header">
          ...
          <div className="ui-auth-intro">
            <h1 className="ui-auth-title" id={`${formId}-title`}>Buat akun baru</h1>
            <p className="ui-auth-subtitle">Rencanakan hari ini, capai bersama.</p>
          </div>
        </header>

        {error && (
          <div id={`${formId}-error`} role="alert" className="ui-auth-error animate-in fade-in duration-300">
            {error}
          </div>
        )}
