...
import { Send, Bot, User, ArrowRight, BrainCircuit, CheckCircle2, AlertCircle, Lightbulb, Sparkles } from 'lucide-react';
...
export default function AIAdvisor() {
    ...
    const healthScoreData = useMemo(() => {
        ...
        // Health Level Text
        let level = 'Cukup';
        let colorClass = 'ui-tone-warning';
        let barColor = 'bg-amber-500';
        
        if (finalScore >= 80) {
            level = 'Sangat Sehat';
            colorClass = 'ui-tone-positive';
            barColor = 'bg-emerald-700';
        } else if (finalScore < 50) {
            level = 'Kritis';
            colorClass = 'ui-tone-danger';
                    ...
                    <div className="ui-card ai-health-panel min-w-0 lg:col-span-4 flex flex-col gap-4 overflow-y-auto">
                        <div>
                            <div className="flex items-center gap-2">
                                <span className="ui-tone-icon ui-tone-ai flex h-8 w-8 shrink-0 items-center justify-center rounded-lg">
                        ...
                        <div className="ai-smart-tip mt-auto pt-4 border-t border-stone-100 flex flex-col gap-2 min-w-0">
                            <div className="flex justify-between items-center">
                                  <span className="ui-badge ui-tone-ai inline-flex items-center gap-1.5">
                                     <Lightbulb className="h-3.5 w-3.5" aria-hidden="true" />
                                     Tips Keuangan
                                </span>
                                <button 
                                    onClick={() => setCurrentTipIndex((prev) => (prev + 1) % smartTips.length)}
                                     className="ai-tip-button ui-button-compact inline-flex items-center text-slate-500 hover:text-violet-700 transition-colors"
                                >
                                    Tip Lainnya
                        ...
                        <div className="ai-chat-messages min-h-0 min-w-0 flex-1 p-4 overflow-y-auto bg-slate-50/50" aria-live="polite">
                            {messages.length === 0 ? (
                                <div className="ai-chat-welcome h-full min-w-0 flex flex-col items-center justify-center px-5 py-8 text-center">
                                    <Sparkles className="mb-3 h-8 w-8 text-violet-700" aria-hidden="true" />
                                    ...
                                    <div className="ai-prompt-list flex flex-wrap justify-center gap-2 max-w-2xl mt-5 min-w-0">
                                        {quickPrompts.map((p) => (
                                            <button
                                                ...
                                                disabled={isTyping}
                                                 className="ai-prompt ui-button-compact max-w-full bg-white hover:bg-violet-50 hover:text-violet-800 border border-stone-200 hover:border-violet-300 text-slate-600 inline-flex items-center gap-1.5 disabled:opacity-50 disabled:pointer-events-none"
                                            >
                                                {p.label}
                                        ...
                                        return (
                                            <div key={msg.id} className={`ai-message-row min-w-0 flex items-start gap-3 ${isBot ? '' : 'flex-row-reverse'}`}>
                                                 <div className={`w-8 h-8 rounded-xl flex items-center justify-center shrink-0 border shadow-sm ${
                                                     isBot ? 'ui-tone-ai' : 'bg-slate-800 border-slate-700 text-white'
                                                }`}>
                                                 ...
                                                 <div className={`ai-message-bubble min-w-0 max-w-[78%] md:max-w-[68%] p-3 rounded-xl text-sm shadow-sm whitespace-pre-line leading-relaxed ${
                                                     isBot ? 'ui-tone-ai rounded-tl-sm' : 'bg-emerald-800 text-white rounded-tr-sm'
                                                }`}>
                            ...
                            {isTyping && (
                                <div className="ai-message-row min-w-0 flex items-start gap-3">
                                     <div className="ui-tone-ai w-8 h-8 rounded-xl flex items-center justify-center shrink-0 border shadow-sm">
                            ...
                            <div className="ai-chat-suggestions min-w-0 px-4 py-2.5 bg-white border-t border-stone-100 flex flex-wrap gap-2 shrink-0">
                                {quickPrompts.map((p) => (
                                    <button
                                        ...
                                        onClick={() => handleSend(p.text)}
                                        disabled={isTyping}
                                         className="ai-prompt ui-button-compact max-w-full bg-slate-50 hover:bg-violet-50 hover:text-violet-800 border border-stone-200 hover:border-violet-300 text-slate-600 transition-all inline-flex items-center gap-1.5 disabled:opacity-50 disabled:pointer-events-none"
                                    >
                                        {p.label}
                                        <ArrowRight className="w-3.5 h-3.5" aria-hidden="true" />
                                    </button>
                        ...
                        >
                            ...
                            <button
                                ...
                                aria-label="Kirim pertanyaan"
                                 className="ai-chat-send ui-button ui-button-icon inline-flex items-center justify-center bg-violet-700 hover:bg-violet-800 text-white transition-colors disabled:bg-slate-200 disabled:text-slate-400 shrink-0"
                            >
