...
export default function MultiWallet() {
    ...
    const renderCategoryCell = (t) => {
        if (t.type === 'transfer') {
            return (
                <span className="ui-badge bg-sky-50 text-sky-700 border-sky-100 inline-flex items-center gap-1">
            ...
            <div className="app-page">
                ...
                <div className="app-page-header">
                    ...
                    <div className="grid grid-cols-2 sm:flex sm:flex-wrap items-center gap-3 w-full xl:w-auto xl:justify-end">
                        ...
                        <button
                            onClick={exportCSV}
                            className="ui-button min-w-0 bg-slate-700 hover:bg-slate-800 text-white inline-flex items-center justify-center gap-2.5"
                        >
                        ...
                        <button
                            onClick={openAdd}
                            className="ui-button min-w-0 bg-emerald-800 hover:bg-emerald-700 text-white inline-flex items-center justify-center gap-2.5"
                        >
                ...
                <div className="grid grid-cols-[repeat(auto-fit,minmax(230px,1fr))] gap-4">
                    <div className="ui-stat-card flex items-center gap-3 min-w-0 bg-stone-50">
                        <div className="w-10 h-10 bg-emerald-800/10 rounded-lg flex items-center justify-center shrink-0">
                    ...
                    <div className="ui-stat-card flex items-center gap-3 min-w-0 bg-stone-50">
                        <div className="w-10 h-10 bg-gray-400/30 rounded-lg flex items-center justify-center shrink-0">
                    ...
                    <div className="ui-stat-card flex items-center gap-3 min-w-0 bg-stone-50">
                        <div className="w-10 h-10 bg-rose-200/30 rounded-lg flex items-center justify-center shrink-0">
                            <svg width="20" height="12" viewBox="0 0 20 12" fill="none">
                        ...
                        <div className="min-w-0 flex-1">
                            ...
                            <span className="block text-red-700 text-lg font-semibold leading-6 truncate">- {fmtIDR(stats.expense)}</span>
                        </div>
                    </div>
                </div>
                ...
                {/* Wallet Management */}
                    ...
                    <div className="grid grid-cols-[repeat(auto-fit,minmax(220px,1fr))] gap-4">
                        {walletAlloc.map((wallet) => {
                            const usage = walletUsage[wallet.id] || { transactions: 0, rules: 0 };
                            return (
                                 <div key={wallet.id} className="ui-card-subtle p-4 flex flex-col gap-4 min-w-0">
                ...
                <div className="ui-card p-5">
                    <div className="flex justify-between items-center mb-4 flex-wrap gap-3">
                        ...
                        <button
                            onClick={() => setRecurringOpen(true)}
                            className="ui-button bg-stone-100 hover:bg-stone-200 border border-stone-300 text-zinc-900"
                        >
                            + Aturan Baru
                        </button>
                    </div>
                                 ...
                                 <div key={rule.id} className={`rounded-lg outline outline-1 outline-offset-[-1px] p-3 flex flex-col gap-2.5 ${rule.active ? 'outline-stone-200 bg-stone-50/50' : 'outline-stone-200 bg-slate-50 opacity-60'}`}>
                                    <div className="flex justify-between items-start gap-2">
                                        <div className="min-w-0">
                                         ...
                                         <span className={`ui-badge shrink-0 ${rule.type === 'income' ? 'bg-emerald-100 text-emerald-700' : 'bg-red-50 text-red-600'}`}>
